...
- [ ] **B3/B4/B5/B6 — Make the fork target an actual blank template.** Strip `data/members.yaml`, `data/projects.yaml`, `TOOLS.md`, `.well-known/*.json`, `SOUL.md`, `IDENTITY.md`, `federation.yaml` to genuine placeholders in `org-os-template` specifically; keep the maintainer's live content in a separate self-hosting repo/branch. **Not a mechanical fix — a positioning-level trade-off.** Splitting the fork target from the maintainer's live content would end "the framework repo is itself a running org-os instance (self-hosting since 2026-04-24)" — a headline proof point at `docs/POSITIONING.md:25` (and the numbers box) and `README.md:28`. Needs its own `DECISIONS.md` entry before execution, not just an implementation pass.
...
- [ ] **M4 — Fix or reframe `clone-framework.mjs`'s "Recommended" framing.** It contradicts README's "fork a repo, answer six questions" — it hand-authors a YAML config and materializes the org in a new sibling directory, not the forked repo itself. **→ moved into v0.5: masterplan WS-I task I1 (clone:framework becomes THE recommended path, documented as such).**
