...
- [2026-08-29] **Interfaces consolidation designed + queued (v0.6 Active #6)** — spec `docs/superpowers/specs/2026-08-29-org-os-interfaces-design.md`: two-plane contract (git-native data plane · admin Hono API as the framework's only server, "new interfaces are clients, never servers"), four canonical surfaces (CLI · KMS · Admin · Conversational), prune manifest for `packages/{dashboard,agents-app,paperclip-agents-app,webapps}` + orphaned `PAPERCLIP_DEPLOYMENT_GUIDE.md`, packages-matrix lifecycle updates. Overturns the 08-02 "stay npm workspaces" ruling; deletions ride `feat/interfaces-prune` after the 0.5.1 cut, targeting 0.6.0. See DECISIONS.md 2026-08-29 + `memory/2026-08-29.md`.
...
_Last updated: 2026-08-29 (interfaces consolidation specced + queued as v0.6 Active #6; v0.5.0 tagged — see the [release masterplan](docs/superpowers/plans/2026-08-28-v0.5-release-masterplan.md) for workstream state)_
