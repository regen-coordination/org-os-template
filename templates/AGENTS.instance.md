# {{ org.name }} — Agent Guide

_Operating instructions for AI agents (Claude Code, Cursor, OpenCode, Hermes, or custom runtimes) working in this org-os instance. {{ org.name }} runs on the org-os framework ({{ framework.url }}); this file covers operating the organization, not developing the framework._
...
This instance can take part in a federation network (see `federation.yaml`).

### Key Implications

- Peer nodes share skills — pull updates when available
- Meeting summaries may be published to a knowledge commons
...
## 11. Upstream Framework

**Upstream:** [org-os-template]({{ framework.url }}) — the canonical framework repository, and the only URL this instance's `upstream` remote should carry. `npm run sync:upstream` pulls framework updates. Changes to the framework itself are proposed upstream, not made here.

**Common integrations** (declare yours in `federation.yaml`): agent runtimes, knowledge infrastructure, publishing, governance (e.g. Gardens, Snapshot, Hats) and treasury tooling (e.g. Safe).

---

## 12. For First-Time Setup

1. Follow `BOOTSTRAP.md` — the first-session interview populates `data/*.yaml`
2. Fill in core identity files:
   - `SOUL.md` — the organization's values and mission
   - `IDENTITY.md` — organization details, chain info, treasury
   - `USER.md` — operator preferences and context
3. Configure `federation.yaml` peers for your network
4. Run `npm run generate:schemas` to create EIP-4824 outputs
...
```bash
npm run initialize         # Visual dashboard of workspace state (or /initialize)
npm run generate:schemas   # Regenerate EIP-4824 schemas
npm run validate:schemas   # Validate schema compliance
npm run selftest           # Validators + test suite
npm run sync:upstream      # Pull framework updates
npm run clone:repos        # Clone repositories listed in repos.manifest.json
```
