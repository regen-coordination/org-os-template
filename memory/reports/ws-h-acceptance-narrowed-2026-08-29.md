# WS-H Acceptance (narrowed claim) — 2026-08-29, v0.5.0 ship day

> Filed under this name rather than `instances-drift-2026-08-29.md`: that
> filename is owned by `npm run analyze:instances`, which regenerates it on
> every gate run and overwrote this report once. The masterplan's H4 wording
> ("overwriting the auto-generated report is fine") only holds until the next
> `analyze:instances` — a hand-written record needs a name the generator does
> not own.
