...
| Berd | **built, catalogued `in-dev`** (2026-08-28 spec; built on `feat/v06-integrations` as v0.6 Active #5) | github.com/block/berd — Block's desktop app over Goose/ACP, open-sourced 2026-08-19 (its "formalize when it stabilizes" trigger fired). Module #4 `org-os-berd` = personas + curated skills bridge into `.agents/skills/`; personas layer verified live, skills bridge built and tested but Goose-verification acceptance still pending (HEARTBEAT.md tracker). Spec: [berd design](../superpowers/specs/2026-08-28-berd-integration-design.md) |
| Buzz | **built, catalogued `in-dev`** (2026-08-28 spec; built on `feat/v06-integrations` as v0.6 Active #4) | github.com/block/buzz — Block's Nostr-based workspace (chat+forge+workflows, one signed event log; humans and agents as keypair peers). Agent-lane bridge = module #3 `org-os-buzz`, fail-open `/close`-posts / `/initialize`-reads hooks built and tested; dogfood acceptance still pending (HEARTBEAT.md tracker) — `CLI_MAP` remains unverified against the real `buzz-cli`. Spec: [buzz design](../superpowers/specs/2026-08-28-buzz-integration-design.md) |
