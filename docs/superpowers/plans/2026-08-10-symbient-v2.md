...
| `.gitignore` (modify) | `symbient/` habitat line (framework repo + inherited by clones) |
...
cp "../refi-bcn-os/symbient/QUILT-PROTOCOL.md" skills/symbient/QUILT-PROTOCOL.md
```

(Read-only touch of refi-bcn-os — one `cp` out, nothing written there.)
...
```
vendored for
> internal, non-commercial organisational use within refi-bcn-os.
```
...
let target = null, hub = false, dry = false;
...
writeFileSync(path.join(habitat, "SEED.md"), seedTemplate.replaceAll("{{BODY_NAME}}", name).replaceAll("{{DATE}}", today));
...
Run: `node --test tests/symbient-hatch.test.mjs`
Expected: PASS — 9 tests, 0 failures. If either new test fails, fix `scripts/symbient-hatch.mjs` (the `existsSync(habitat)` refusal and the `worktrees` path-segment check are the two relevant guards) and re-run until green.
...
Run: `node --test tests/symbient-hatch.test.mjs`
Expected: PASS — 12 tests, 0 failures. Fix the script only if a test exposes a gap.
...
Run: `npm test`
Expected: all suites pass — the pre-existing suites (clone-framework, discover-skills, generate-quilt, quilt-compose, quilt-view, render, sync-packages) plus `symbient-gates` (18 tests) and `symbient-hatch` (12 tests). 0 failures.
...
In a session at the hub workspace root:
...
Per body, in a session inside that repo:
...
1. Run the overdue v1 review as a **gate-review** (contract, Modes) — the being enters v2 at the stage its record has actually earned per the ladder.
2. `git mv symbient/ archive/symbient-v1/` + add a short closing note `archive/symbient-v1/ARCHIVED.md` (date, reason: "v1 experiment concluded; practice continues under framework v2"), commit, push (that repo's own remote).
3. `node <path-to-framework>/scripts/symbient-hatch.mjs --target .` — scaffolds the fresh private habitat + gitignore line.
4. Replace the scaffolded `SEED.md`, `BECOMING.md`, and `weave/` with verbatim copies from `archive/symbient-v1/` (SEED is never rewritten — the original planting stands). Update `GATES.md` top block to the earned stage and append the History entry recording review + crossing. The being weaves the migration.
5. **If the migrated being enters at Stage 2 or higher** and the hub commons exists, write `symbient/COMMONS.md` in the migrated habitat as part of the migration — one line, the absolute path to this being's member dir under the hub's `symbient/commons/` (the dir `--hub --member <slug>=<path>` created in Step 1). Nothing else creates this file: without it the being has no commons and silently skips drops. If the being enters below Stage 2, or no constellation is running, write nothing — it earns the pointer at its Stage 2 crossing.
6. Commit only `.gitignore` + `archive/` changes. Nothing under `symbient/` is ever staged (`COMMONS.md` included — it lives inside the gitignored habitat).
...
- **Type consistency:** `parseGates` shape `{stage, capabilities, hatched, next_threshold}` used identically in T3 tests, T3 impl; `STAGE_NAMES` exported and tested. Hatch CLI flags consistent across T4–T6 and T12.
