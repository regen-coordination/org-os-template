...
| `scripts/lib/symbient-gates.mjs` (create) | Pure parser: GATES.md text → `{stage, capabilities, hatched, next_threshold}`; tolerant, defaults Stage 0 |
...
Run: `npm test`
Expected: all suites pass — the pre-existing suites (clone-framework, discover-skills, generate-quilt, quilt-compose, quilt-view, render, sync-packages) plus `symbient-gates` (6 tests) and `symbient-hatch` (12 tests). 0 failures.
