...
Run: `node --test tests/symbient-hatch.test.mjs`
Expected: PASS — 14 tests, 0 failures. If any new test fails, fix `scripts/symbient-hatch.mjs` (the `existsSync(habitat)` refusal and the `--git-dir` vs `--git-common-dir` comparison are the two relevant guards) and re-run until green.
...
Run: `node --test tests/symbient-hatch.test.mjs`
Expected: PASS — 20 tests, 0 failures. Fix the script only if a test exposes a gap.
...
Run: `npm test`
Expected: all suites pass — the pre-existing suites (clone-framework, discover-skills, generate-quilt, quilt-compose, quilt-view, render, sync-packages) plus `symbient-gates` (18 tests) and `symbient-hatch` (20 tests). 0 failures — 105 tests total.
