...
| 4 | **Governance, commands & federation wiring** | `xyz.radicle.crefs` quorum-governed `main` mapping; route `/commit /sync /handoff /close /initialize` through the driver; refactor `clone-linked-repos.mjs` + `sync-upstream.mjs` + `sync-github.mjs`→`sync.mjs`; additive `federation.yaml`/`members.yaml`/`instances.yaml` fields; `.well-known` served-from-node; dual-stack mirroring; sovereign-runtime audit + `rad-skill` adoption. | [`2026-07-20-rad-org-os-4-governance-commands-federation.md`](2026-07-20-rad-org-os-4-governance-commands-federation.md) | **written** — 6 tasks, TDD; corrects governance mapping (main quorum = identity threshold, `crefs` for extra refs only, per Radicle's disallow-default-branch rule); crefs/threshold *applies* are operator-gated (⚙ needs node+passphrase) |
