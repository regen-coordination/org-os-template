...
| 3 | **Bootstrap & seed-node** | `rad-bootstrap` (zero→live Radicle-first genesis) + seed-node recipe (`Dockerfile`/`compose.yml`/`compose.tor.yml`/`seeding-policy.md`) + the 3-tier availability chooser. | [`2026-07-20-rad-org-os-3-bootstrap-seed-node.md`](2026-07-20-rad-org-os-3-bootstrap-seed-node.md) | **✅ shipped** — merged to `v0.5` (8 commits; `rad-bootstrap` git-inits + genesis-commits before `rad init`, did+rid guarded, fail-loudly; 3-tier availability chooser; seed-node Docker+Tor recipe; 48 rad tests + 3 gated-skip). Review caught & fixed a Critical (missing git-init) + the dash Dockerfile bug. Remaining: run the gated live-bootstrap end-to-end (`rad node start` + creates a real repo — needs operator go-ahead). |
| 4 | **Governance, commands & federation wiring** | `xyz.radicle.crefs` quorum-governed `main` mapping; route `/commit /sync /handoff /close /initialize` through the driver; refactor `clone-linked-repos.mjs` + `sync-upstream.mjs` + `sync-github.mjs`→`sync.mjs`; additive `federation.yaml`/`members.yaml`/`instances.yaml` fields; `.well-known` served-from-node; dual-stack mirroring; sovereign-runtime audit + `rad-skill` adoption. | [`2026-07-20-rad-org-os-4-governance-commands-federation.md`](2026-07-20-rad-org-os-4-governance-commands-federation.md) | **✅ shipped** — merged to `v0.5` (9 commits; governance mapping, committed genesis stamp, member-id-scheme validator, rid routing in clone/sync scripts, canonical-aware `/commit /sync /handoff /close /initialize`, data-model docs + `RADICLE.md` + `rad-skill` candidate + clean runtime audit; 55 rad + 30 host + 66 kms tests). Operator-gated ⚙: applying `crefs`/threshold via `rad id update`. |
