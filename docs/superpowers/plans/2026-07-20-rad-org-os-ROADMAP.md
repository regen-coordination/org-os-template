...
|---|------|-------|------|--------|
| 1 | **Host abstraction foundation** | `@org-os/host` package: `HostDriver` interface + registry, driver resolver keyed on `platforms.canonical`, behavior-preserving `github` driver, a reusable driver-contract test suite, and `frontier.mjs` refactored to route through the seam. GitHub cohort behavior unchanged. | [`2026-07-20-rad-org-os-1-host-abstraction.md`](2026-07-20-rad-org-os-1-host-abstraction.md) | **written** |
...
| 3 | **Bootstrap & seed-node** | `rad-bootstrap` (zero→live Radicle-first genesis) + seed-node recipe (`Dockerfile`/`compose.yml`/`compose.tor.yml`/`seeding-policy.md`) + the 3-tier availability chooser. | `2026-07-20-rad-org-os-3-bootstrap-seed-node.md` | pending |
