...
### `.agents/` — Berd Agent Surfaces (Optional)
...
└── skills/                 # Berd/Goose project-local Agent Skills discovery path
    ├── org-os-init/        # materialized — sync:skills:berd owns these five
    ├── meeting-processor/
    ├── heartbeat-monitor/
    ├── knowledge-curator/
    ├── funding-scout/
    └── feynman/            # hand-authored — the sync never touches it
```

`.agents/agents/` is the canonical source for the workspace's Berd personas, shipped
2026-08-20. Berd discovers these automatically when the project is open;
`npm run sync:agents` mirrors them to the user-level `~/.agents/agents/` so they work
across all workspaces.

`.agents/skills/` holds materialized, curated skills — generated by
`npm run sync:skills:berd` from the exposure list in `modules/org-os-berd/module.yaml`,
committed to the repo, and drift-gated (`--check`, wired into `npm run selftest`). It may
also hold hand-authored skill directories (e.g. `feynman/`); the sync only manages the
targets it wrote (marked `managed_by: org-os`) and never touches anything else. See
