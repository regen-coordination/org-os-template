...
- **Step 2/3 (branch logic + fast-forward):** unchanged in spirit — still never fast-forward over uncommitted changes. Where github compares against `@{u}`/`origin/main`, radicle compares against the canonical branch — whose NAME comes from `driver.getDrift(rid).canonicalRef` (e.g. `rad/main`), the ahead/behind computed locally (next point).
- **Step 4 (drift vs main):** the ahead/behind counts come from a local `git rev-list --left-right --count <canonicalRef>...HEAD` against the canonical branch in your checkout (exactly as `scripts/sync-upstream.mjs` computes drift) — `driver.getDrift(rid)` supplies only the canonical branch *name*, not live counts (it has an rid but no working copy, so its counts are always 0). Use `driver.webUrl(rid, path)` links (instead of a `github.com/.../blob/main/...` URL) when naming which shared files the canonical branch has that this trunk lacks. A merge of the canonical branch in, when behind, still needs explicit operator consent and a vault snapshot first — same as the github path.
