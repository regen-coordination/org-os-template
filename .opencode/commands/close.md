...
2. **Write memory** — Append a session entry to `memory/YYYY-MM-DD.md` (today's date). Create the file if it doesn't exist. Never overwrite. Include: focus area, key decisions, actions taken, what remains.
...
5. **Update plan queue** — If any plan in `docs/agent-plans/` changed status (started, completed, tasks checked off), update the plan file and `docs/agent-plans/QUEUE.md`.
...
Stage `memory/`, `HEARTBEAT.md`, `MEMORY.md`, `data/`, and `docs/agent-plans/` — then commit with a descriptive message like `session: [focus area summary]`. Show the result.
